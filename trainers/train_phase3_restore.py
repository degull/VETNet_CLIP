...
class Config:
    ...
    # checkpoints
    save_root: str = "E:/VETNet_CLIP/checkpoints/phase3_restore_AR"
    # previews
    results_root: str = "E:/VETNet_CLIP/results/phase3_restore_AR"
...
def train():
    ...
    translator_cfg = ConditionTranslatorConfig(
        ...
        film_mode=cfg.film_mode,
        debug_ar=True,        # ✅ 추가
        debug_every=200       # (원하면 50으로 줄여도 됨)
    )
    translator = ConditionTranslator(translator_cfg).to(device)

    print(
    "[DEBUG] Translator params:",
    sum(p.numel() for p in translator.parameters())
)
